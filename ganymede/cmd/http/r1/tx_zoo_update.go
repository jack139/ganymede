...
func TxZooUpdate(ctx *fasthttp.RequestCtx) {
	...
	// 修改链上数据
	respData, err := itemModify((*zooData)["creator"].(string), ownerAddr, zooKey, zooValue, crypto)
	if err != nil {
		if strings.Contains(err.Error(), "account sequence mismatch") {
			helper.RespError(ctx, 9090, "Same user's TX busy! please try again later.")
		} else {
			helper.RespError(ctx, 9099, "update fail: " + err.Error())
		}
		return
	}
...
// 修改链上数据，send msg
func itemModify(callerAddr string, ownerAddr string, zooKey string, zooValue string,
		crypto bool) (*map[string]interface{}, error) {
	...
	// 数据上链
	msg := invtypes.NewMsgUpdateKvzoo(
