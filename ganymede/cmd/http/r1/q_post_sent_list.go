...
/* 查询 跨链 sent 清单 */
func QueryPostSentList(ctx *fasthttp.RequestCtx) {
	...
	// 查询链上数据
	respData2, err := queryPostSentListPage(uint64(page), uint64(limit), senderAddr, targetAddr, askUuid)
	if err!=nil{
	...
	for _, item0 := range dataList {
		...
		if strings.HasPrefix(item["title"].(string), "@EXCH:ASK:") {
			...
			newItem["ask_post_id"]  = item["postID"]
			...
			newItem["reply_post_id"] = item["postID"]
...
// 查询链上数据, 返回 map
func queryPostSentListPage(page uint64, limit uint64, 
	...
	if len(senderAddr)==0 && len(targetAddr)==0 { // 查所有的
		...
		params := &posttypes.QueryAllSentPostRequest{
			Pagination: &pageReq,
		}
		...
		params := &posttypes.QueryListSentBySenderRequest{
			Page: page,
			Limit: limit,
			Sender: senderAddr + askUuid,
		}		

		res, err := queryClient.ListSentBySender(context.Background(), params)
		if err != nil {
			return nil, err
		}
		...
		params := &posttypes.QueryListSentByReceiverRequest{
			Page: page,
			Limit: limit,
			Receiver: targetAddr + askUuid,
		}		

		res, err := queryClient.ListSentByReceiver(context.Background(), params)
		if err != nil {
			return nil, err
		}
...
// 查询链上数据, 返回 map
func queryPostSentInfoById(postId uint64) (*map[string]interface{}, error) {
	// 获取 ctx 上下文
	clientCtx := client.GetClientContextFromCmd(helper.HttpCmd)
	...
	params := &posttypes.QueryGetSentPostRequest{
		Id: postId,
	}
	...
	itemMap := respData["SentPost"].(map[string]interface{})
