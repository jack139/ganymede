...
func (k Keeper) ListAskByReplier(goCtx context.Context, req *types.QueryListAskByReplierRequest) (*types.QueryListAskByReplierResponse, error) {
	...
	storeLink := prefix.NewStore(store, types.KeyPrefix(types.AskReplierLinkPrefix))

	// 解析参数: sender|askId
	params := strings.Split(req.Replier, "|")
	if len(params)!=2 { // 参数出错
		return &types.QueryListAskByReplierResponse{Ask: asks}, nil
	}

	// 找到 replier 的尾部
	linkKey := types.ExchangeLinkKey("replier", params[0]) // 链表头 key
	...
	for {
		...
		if ask.LinkReplier == "@@LINK:$" {
			break
		}
		kvKey = []byte(ask.LinkReplier)
	}
