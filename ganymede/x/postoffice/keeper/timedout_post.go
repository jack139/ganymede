...
func (k Keeper) AppendTimedoutPost(
	...
	idByteKey := GetTimedoutPostIDBytes(timedoutPost.Id)

	// 检索链表头信息, 2个链表： sender, receiver
	senderKey := types.PostofficeLinkKey("timeoutSender", timedoutPost.Sender) // 链表头 key
	storeSenderLink := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.TimeoutSenderLinkPrefix))
	b := storeSenderLink.Get(senderKey)
	if b == nil { // 未找到，说明是新的值
		timedoutPost.LinkSender = "@@LINK:$" 
	} else {
		timedoutPost.LinkSender = string(b)
	}
	storeSenderLink.Set(senderKey, idByteKey)  // 保存表头数据

	receiverKey := types.PostofficeLinkKey("timeoutReceiver", timedoutPost.Receiver) // 链表头 key
	storeReceiverLink := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.TimeoutReceiverLinkPrefix))
	b = storeReceiverLink.Get(receiverKey)
	if b == nil { // 未找到，说明是新的值
		timedoutPost.LinkReceiver = "@@LINK:$" 
	} else {
		timedoutPost.LinkReceiver = string(b)
	}
...
// GetTimedoutPost returns a timedoutPost from its id
func (k Keeper) GetTimedoutPostByKey(ctx sdk.Context, key []byte) (val types.TimedoutPost, found bool) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.TimedoutPostKey))
	b := store.Get(key)
	if b == nil {
		return val, false
