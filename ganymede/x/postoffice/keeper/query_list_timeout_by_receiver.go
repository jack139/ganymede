...
func (k Keeper) ListTimeoutByReceiver(goCtx context.Context, req *types.QueryListTimeoutByReceiverRequest) (*types.QueryListTimeoutByReceiverResponse, error) {
	if req == nil {
	...
	var posts []*types.TimedoutPost

	store := ctx.KVStore(k.storeKey)
	storeLink := prefix.NewStore(store, types.KeyPrefix(types.TimeoutReceiverLinkPrefix))

	// 解析参数: Receiver|askUuid
	params := strings.Split(req.Receiver, "|")
	if len(params)!=2 { // 参数出错
		return &types.QueryListTimeoutByReceiverResponse{TimedoutPost: posts}, nil
	}

	// 找到 Receiver 的尾部
	linkKey := types.PostofficeLinkKey("timeoutReceiver", params[0]) // 链表头 key
	...
	for {
		post, found := k.GetTimedoutPostByKey(ctx, kvKey)
		if !found {
