...
func (k Keeper) ListTimeoutBySender(goCtx context.Context, req *types.QueryListTimeoutBySenderRequest) (*types.QueryListTimeoutBySenderResponse, error) {
	if req == nil {
	...
	storeLink := prefix.NewStore(store, types.KeyPrefix(types.TimeoutSenderLinkPrefix))

	// 解析参数: sender|askUuid
	params := strings.Split(req.Sender, "|")
	if len(params)!=2 { // 参数出错
		return &types.QueryListTimeoutBySenderResponse{TimedoutPost: posts}, nil
	}

	// 找到 sender 的尾部
	linkKey := types.PostofficeLinkKey("timeoutSender", params[0]) // 链表头 key
